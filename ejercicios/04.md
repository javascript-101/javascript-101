...
* División (`/`): divide ambos valores a los lados del operador.
...
* Igualdad (`==`): devuelve `true` si ambos valores son verdaderos.
...
* Menor o igual (`<=`): devuelve `true` si el valor de la izquierda es menor o igual que el valor de la derecha.
    ...
    console.log(esMenorOIgual); // false
    ```
...
1. Declarar una variable de nombre `miAnimalFavorito` y asignarle como valor el string que represente el animal que más te gusta. Escribir el código necesario para mostrar por la terminal si el animal que más te gusta es un `'perro'`. Usar el operador de igualdad (`==`).
